...
echo '<h1>Liste des fichiers DBC</h1>';
...
while($file = readdir($dir)) 
{
	if($file != '.' && $file != '..' && !is_dir($dirname.$file))
	{
		...
		if ($extension['extension'] == "dbc")
		{
			echo '- Fichier <a href="search.php?dbc=' . $extension['filename'] . '">' . $extension['filename'] . '</a>';
			try
			{
				$dbc = new DBC('./dbcs/' . $extension['filename'] . '.dbc', DBCMap::fromINI('./maps/' . $extension['filename'] . 'Entry.ini'));
				echo ' (' . $dbc->GetRecordCount() . ' entrées)<br />';
				unset($dbc);
			}
			catch(DBCException $e)
			{
				echo ' (entrées illisibles, le fichier semble corrompu)<br />';
			}
		}
	}
}

/* Dump of DBCStructure.h
...
struct CharStartOutfitEntry
{
    ...
    int32 ItemId[24];                         				// 2-13
    //int32 ItemDisplayId[24];               				// 14-25 not required at server side
    //int32 ItemInventorySlot[24];           				// 26-37 not required at server side
